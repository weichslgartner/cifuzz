...
func NewMinijail(opts *Options) (*minijail, error) {
	...
	// Required by the script generated by bazel via --script_path
	envPath, err := exec.LookPath("env")
	if err != nil {
		return nil, errors.WithStack(err)
	}
	bindings = append(bindings, &Binding{Source: envPath})
	bindings = append(bindings, &Binding{Source: "/bin/bash"})
