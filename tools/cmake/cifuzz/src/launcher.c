 ...
 *
 * Instead, we set the CIFUZZ_IN_CIFUZZ environment variable in cifuzz to signal to the fuzz test that it is already
...
void __msan_scoped_disable_interceptor_checks() {
  ...
  printf("If you really want to start the raw fuzzer binary, set CIFUZZ_IN_CIFUZZ=1.\n");
  exit(1);
}
